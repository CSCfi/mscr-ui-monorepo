...
import { TooltipText } from '@app/modules/crosswalk-editor/mappings-accordion/function-tooltip-box/function-tooltip-box.styles';
...
export default function FunctionTooltipBox(props: {
  ...
  function generateTexts() {
    ...
    if (props.functionName === 'predicate') {
      ...
      const mappingFunction = props.mappingFunctions.filter(
        (fnc) => fnc.uri === props.processingId
      );
      ...
      if (props.functionName === 'sourceOperation') {
        node = props.row.source.filter(
          (node) => node.id === props.processingId
        );
      } else {
        node = props.row.target.filter(
          (node) => node.id === props.processingId
        );
      }
  ...
  return (
    <>
      <Tooltip
        ...
        title={
          tooltipOpen
            ? 'Hide ' + props.tooltipHoverText + ' details'
            : 'Show ' + props.tooltipHoverText + ' details'
        }
        placement="bottom"
      >
        <IconCircleMid onClick={() => tooltipClick()}>
          ...
          {!props.alternateIconLetter && <FunctionIcon></FunctionIcon>}
        </IconCircleMid>
      </Tooltip>
      <StooltipContainer>
        <Stooltip
        ...
        >
          <Heading variant="h5" as="h2">
            {props.tooltipHeading[0].toUpperCase() +
              props.tooltipHeading.slice(1)}
          </Heading>
          {tooltipOpen && generateTexts()}
          <Tooltip
            title={
              props.isEditModeActive
                ? 'Edit ' + props.tooltipHeading
                : 'Activate edit mode to edit ' + props.tooltipHeading
            }
            placement="bottom"
          >
            <span>
