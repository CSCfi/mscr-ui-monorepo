...
function getMappingFunctionIOFormats(
  functionId: string,
  mappingFunctions: any
) {
  let IOFormats = { input: '', output: '' };

  const functions = mappingFunctions.filter(
    (item: any) => item.uri === functionId
  );
  ...
  if (functions[0]?.outputs) {
    functions[0]?.outputs[0]?.datatype
      ? (IOFormats.output = functions[0]?.outputs[0]?.datatype)
      : undefined;
  }
  return IOFormats;
}
...
export function validateMappings(
  mappingNodes: CrosswalkConnectionNew[],
  mappingFunctions: any
) {
  ...
  mappingNodes.forEach((node) => {
    if (node.sourceProcessing) {
      ...
      const sourceProcessingIOFormats = getMappingFunctionIOFormats(
        node.sourceProcessing.id,
        mappingFunctions
      );
      if (node.source.properties.type !== sourceProcessingIOFormats.input) {
        errorMessages.push(
          'Datatype mismatch: ' +
            node.source.name +
            ': ' +
            node.source.properties.type +
            ' -> ' +
            node.sourceProcessing.id +
            ': ' +
            sourceProcessingIOFormats.input
        );
      }
      if (node?.processing) {
        // Mapping operation is present
        const mappingFncIOFormats = getMappingFunctionIOFormats(
          node.processing.id,
          mappingFunctions
        );
        if (sourceProcessingIOFormats.output !== mappingFncIOFormats.input) {
          errorMessages.push(
            'Datatype mismatch: ' +
              node.sourceProcessing.id +
              ': ' +
              sourceProcessingIOFormats.output +
              ' -> ' +
              node.processing.id +
              ': ' +
              mappingFncIOFormats.input
          );
        }
        if (node.target.properties.type !== mappingFncIOFormats.output) {
          errorMessages.push(
            'Datatype mismatch: ' +
              node.processing.id +
              ': ' +
              mappingFncIOFormats.output +
              ' -> ' +
              node.target.name +
              ': ' +
              node.target.properties.type
          );
        }
      } else {
...
export default function ValidationErrorBar(props: {
  ...
  function getErrorCaption(validationErrors: string[]) {
    return (
      <div className="d-flex justify-content-between font-weight-normal">
        <div className="">
          {validationErrors.length} datatype validation warnings
        </div>
        <div className="d-flex justify-content-end">
          <div
            className="anchorlink me-4"
            onClick={() => setIsDetailsVisible(!isDetailsVisible)}
          >
            {isDetailsVisible ? 'hide details' : 'show details'}
          </div>
          <Tooltip title="Click to dismiss" placement="bottom">
            <div className="me-1 d-flex flex-column justify-content-center">
              <IconClose
                onClick={() => props.hideErrorBarCallback('hideErrorBar')}
              ></IconClose>
            </div>
          </Tooltip>
        </div>
  ...
  if (props.mappingNodes) {
    validationErrors = validateMappings(
      props.mappingNodes,
      ...
      <div className="mb-2">
        {validationErrors.length > 0 && (
          <StyledInlineAlert
            className={'mt-3'}
            status="warning"
            labelText={getErrorCaption(validationErrors)}
          >
            {isDetailsVisible && (
              <ul>
