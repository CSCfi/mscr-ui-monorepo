...
export default function TreeviewTest({ pid, user }: TreeViewTestProps) {
    ...
    // STATE VARIABLES
    const [sourceTreeData, setSourceData] = React.useState<any>(
        MockupSchemaLoader(true)
    );
    ...
    const [sourceTreeSelectedArray, selectFromSourceTreeByIds] = React.useState<
        string[]
        >([]);
    const [sourceTreeSelection, setSourceSelection] =
        React.useState<RenderTree>(emptyTreeSelection);

    const [targetTreeData, setTargetData] = React.useState<any>(
        MockupSchemaLoader(true)
    );
    ...
    const [targetTreeSelectedArray, selectFromTargetTreeByIds] = React.useState<
        string[]
        >([]);
    const [targetTreeSelection, setTargetSelection] =
        React.useState<RenderTree>(emptyTreeSelection);

    const [currentlySelectedSource, setCurrentlySelectedSource] =
        React.useState<CrosswalkConnection>(crosswalkConnectionInit);
    const [currentlySelectedTarget, setCurrentlySelectedTarget] =
        React.useState<CrosswalkConnection>(crosswalkConnectionInit);
    ...
    useEffect(() => {
        ...
        const sourceNode: CrosswalkConnection = Object.assign(
            {},
            crosswalkConnectionInit
        );
        ...
        const targetNode: CrosswalkConnection = Object.assign(
            {},
            crosswalkConnectionInit
        );
    ...
    const handleExpandClick = (isSourceTree: boolean) => {
        ...
        if (isSourceTree) {
            sourceTreeData.forEach(
                (item: { children: string | any[]; id: string }) => {
                        ...
                        <>
                            ...
                            <Box sx={{ height: 180, flexGrow: 1 }}>
                                <NodeInfo
                                    ...
                                    sourceData={currentlySelectedSource}
                                ...
                                <div className="col-2">
                                    <div className="d-flex align-content-center justify-content-center flex-column h-100">
                                        <div className="d-flex justify-content-center">
                                            <IconButton
                                                onClick={() => addOrRemoveJoint(!isBothSelectedLinked)}
                                                ...
                                                size="large"
                                                disabled={
                                                    (isAnySelectedLinked && !isBothSelectedLinked) ||
                                                    !(
                                                        currentlySelectedSource.source.length > 0 &&
                                                        currentlySelectedTarget.source.length > 0
                                                    )
                                                }
                                            >
                                                {isBothSelectedLinked ? (
