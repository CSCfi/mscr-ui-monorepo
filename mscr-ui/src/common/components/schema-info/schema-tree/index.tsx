...
function returnIconForMappedNode(
  ...
  if (isSourceTree) {
    if (nodeMappings) {
      foundMappings = nodeMappings.find((nodeMapping) =>
        nodeMapping.source.find((sourceItem) => sourceItem.id === node.id)
      );
    }
  } else {
    if (nodeMappings) {
      foundMappings = nodeMappings.find((nodeMapping) =>
        nodeMapping.target.find((targetItem) => targetItem.id === node.id)
      );
    }
  }
  if (foundMappings != null) {
    return (
      <Tooltip
        title={'Schema tree node is used on Crosswalk mapping.'}
        placement="bottom"
      >
        <CircleIcon
          style={{ color: '#1976d2', maxHeight: '25%', maxWidth: '25%' }}
        />
      </Tooltip>
    );
  } else {
  ...
  if (Array.isArray(nodes)) {
        ...
        <div style={{ display: 'flex' }}>
          ...
          >
            {Array.isArray(node.children)
              ? node.children.map((node: RenderTree) =>
                  toTree(node, showQname, nodeMappings, isSourceTree)
                )
              : null}
          </TreeItem>
          {returnIconForMappedNode(nodeMappings, node, isSourceTree)}{' '}
        </div>
      );
    });
  } else {
    ret = (
      <div style={{ display: 'flex' }}>
        ...
        {returnIconForMappedNode(nodeMappings, nodes, isSourceTree)}{' '}
      </div>
    );
    return ret;
  }
